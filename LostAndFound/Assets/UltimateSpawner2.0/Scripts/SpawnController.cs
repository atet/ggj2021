...
namespace UltimateSpawner
{
    ...
    public abstract class SpawnController : MonoBehaviour
    {
        ...
        /// <summary>
        /// Use this method to despawn any alive items that were created by this spawn controller.
        /// </summary>
        public void DespawnAllItems()
        {
            List<SpawnedItem> despawnItems = new List<SpawnedItem>(spawnedItems);

            foreach(SpawnedItem spawnedItem in despawnItems)
            {
                // Check for destroyed already
                if(spawnedItem.spawnedTransform != null)
                {
                    // Despawn the item
                    UltimateSpawning.Despawn(spawnedItem.spawnedTransform);
                }
            }
        }

...
namespace UltimateSpawner.Spawning
{
    public class SpawnBounds : EndPointSpawner
    {
        ...
        [Range(0f, 1f)]
        public float centerExclusion = 0.5f;
        ...
        public override SpawnLocation GetLocation()
        {
            ...
            Vector3 localPoint = transform.position;
            int attempt = 0;

            while (Vector3.Distance(localPoint, Vector3.zero) < centerExclusion && attempt < 50)
            {
                localPoint = SpawnRandomizer.GetRandomPosition(spawnBounds);
                attempt++;
            }
        ...
        public void OnDrawGizmos()
        {
            ...
            Gizmos.DrawCube(spawnBounds.center, spawnBounds.size * centerExclusion);
        }
    }
}

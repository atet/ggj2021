    ...
    public sealed class SpawnArea : EndPointSpawner
    {
        ...
        public void UpdateAvailableSpawnLocations()
        {
            ...
            if (is2DSpawner == true)
            {
                ...
                {
                    // Overlap a box
                    collidersInsideArea = Physics2D.OverlapBoxNonAlloc(transform.position, size / 2, transform.rotation.z, sharedCollider2DBuffer, collisionLayer);
                ...
                {
                    // Get the collider list
                    collidersInsideArea = colliding2D.Count;
                ...
                foreach (SpawnAreaLocation potentialLocation in spawnLocations)
                {
                    ...
                    for (int i = 0; i < collidersInsideArea; i++)
                    {
                        ...
                        // Skip trigger colliders in some cases
                        if (occupiedCheck == OccupiedCheck.PhysicsOverlap)
                        {
                            if (collider.isTrigger == true && allowTriggerOccupiedChecks == false)
                                continue;
                        }
                        else
                        {
            ...
            else
            {
                ...
                if (check == OccupiedCheck.PhysicsOverlap)
                {
                    ...
                    if (isAboveGround == true)
                        heightOffset = ceilingHeight / 2;

                    // Overlap a box
                    collidersInsideArea = Physics.OverlapBoxNonAlloc(transform.position + new Vector3(0, heightOffset, 0), new Vector3(size.x / 2, ceilingHeight / 2, size.y / 2), sharedColliderBuffer, transform.rotation, collisionLayer);
